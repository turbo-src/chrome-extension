...
async function postGetContributorTokenAmount(owner, repo, pr_id, contributor_id, side) {
  const res = await superagent
    .post(`${port}/graphql`)
    .send({
...
async function postGetPRpercentVotedQuorum(owner, repo, issue_id, contributor_id, side) {
  const res = await superagent
    .post(`${port}/graphql`)
    .send(
      //{ query: '{ name: 'Manny', species: 'cat' }' }
